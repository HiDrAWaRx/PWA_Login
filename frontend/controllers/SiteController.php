    ...
    public function actionVerifyEmail($token) {
        ...
        if ($user = $model->verifyEmail()) {
            if (Yii::$app->user->login($user)) {
                Yii::$app->session->setFlash('success', '<p> Tu dirección de correo ha sido verificada. </p> <p> ¡Muchas gracias por registrarte! </p>');
                return $this->goHome();
            }
        }

        Yii::$app->session->setFlash('error', '<p> Lo sentimos, el enlace de confirmación provisto no es válido. </p>.');
        return $this->goHome();
    }
